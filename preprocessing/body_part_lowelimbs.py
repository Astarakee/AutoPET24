...
write_path = '/home/mehdi/Data/AutoPET24/Dataset705_AutoPET24Crop_LowerLimbs'
...
for ix, case in enumerate(label_files):
    ...
    l_hip_path = os.path.join(case_path_totalseg, 'hip_left.nii.gz')
    r_hip_path = os.path.join(case_path_totalseg, 'hip_right.nii.gz')
    ...
    l_hip_array, _, _, _, _, _ = read_nifti(l_hip_path)
    r_hip_array, _, _, _, _, _ = read_nifti(r_hip_path)
    array_shape = ct_array.shape
    hip_array = l_hip_array+r_hip_array
    hip_array[hip_array!=0] = 1

    ax_ind_lower, _, _ = np.where(hip_array==1)
    min_ax_slice_lower = np.min(ax_ind_lower)
    min_ax_marginal_lower = min_ax_slice_lower+axial_margin

    hn_ct_arr = ct_array[:min_ax_marginal_lower,:,:]
    hn_pt_arr = pt_array[:min_ax_marginal_lower,:,:]
    hn_sg_arr = sg_array[:min_ax_marginal_lower,:,:]
