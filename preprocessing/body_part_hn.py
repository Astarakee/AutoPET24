...
write_path = '/home/mehdi/Data/AutoPET24/Dataset705_AutoPET24Crop_HN'
...
for ix, case in enumerate(label_files):
    ...
    l_cl_array, l_cl_itk, _, _, _, _ = read_nifti(l_clavicle_path)
    r_cl_array, r_cl_itk, _, _, _, _ = read_nifti(r_clavicle_path)
    array_shape = ct_array.shape
    clavicle_array = l_cl_array+r_cl_array
    clavicle_array[clavicle_array!=0] = 1
    ax_ind, y_ind, z_ind = np.where(clavicle_array==1)
    min_ax_slice = np.min(ax_ind)
    min_ax_marginal = min_ax_slice-axial_margin
    hn_ct_arr = ct_array[min_ax_marginal:,:,:]
    hn_pt_arr = pt_array[min_ax_marginal:,:,:]
    hn_sg_arr = sg_array[min_ax_marginal:,:,:]
    ...
    temp['start_slice_crop'] = min_ax_marginal
    temp['end_slice_crop'] = array_shape[0]
